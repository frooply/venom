...
module.exports = class Sessions {
    ...
    static async getContatosGrupo(sessionName, grupoid) {
        var session = Sessions.getSession(sessionName);
        let json = [], json1, json2
        let nome
        ...
        if (session) {
            if (session.state == "CONNECTED" || session.state == "CONFLICT" || session.state == "STARTING") { // Corrigir conflito
                var resultSendText = await session.client.then(async client => {
                    ...
                    let grupo = await client.getGroupMembers(grupoid).then(ok=>{return ok})
                    grupo.map(contato=>{
                        console.log(contato)
                        nome = contato.pushname || contato.formattedName || contato.displayName || contato.formattedShortName || contato.formattedShortNameWithNonBreakingSpaces || contato.mentionName || contato.notifyName
                        json.push({
                            nome,
                            contatoId: contato.id._serialized
                        })
                    })
